...
def _mfa_registration_view(
    ...
    current_app.logger.debug(f"Processing MFA registration view for {mfa.name} with form data: {form_data}")
    ...
    if form_data[mfa.name] == 'SETUP':
        ...
        # Clear any existing session data for this method
        if mfa.name == 'sms':
            session.pop('mfa_sms_code', None)
            session.pop('mfa_phone_number', None)
        elif mfa.name == 'email':
            session.pop('mfa_email_code', None)
            session.pop('mfa_email_id', None)
        elif mfa.name == 'authenticator':
...
def registration_view() -> Response:
    ...
    * POST: This handles multiple scenarios on the registration page:
    ...
    # Debug logging for MFA auths
    current_app.logger.debug(f"Available MFA methods: {mfa_auths}")

    next_url = request.args.get("next", None)

    if request.method == 'POST':
        # Debug logging for POST request
        current_app.logger.debug(f"POST request form data: {request.form.to_dict()}")
    ...
    for key in mfa_auths:
        ...
        # Debug logging for each MFA method
        current_app.logger.debug(f"Processing MFA method: {mfa}")
    ...
    # Get the current view if a setup is in progress
    mfa_view = None
    if request.method == 'POST':
        form_data = request.form.to_dict()
        current_app.logger.info(f"MFA Registration POST request received with form data: {form_data}")
        
        # Handle all MFA methods consistently
        for key in mfa_auths:
            mfa = mfa_auths[key]['mfa']
            current_app.logger.info(f"Checking MFA method: {mfa.name} with key: {key}")
            # Check for both auth_method and method name in form data
            if any(k in form_data for k in [mfa.name, key, 'auth_method']):
                current_app.logger.info(f"Found matching MFA method: {mfa.name}")
                # Ensure form_data has the correct structure for setup
                if form_data.get(key) == 'SETUP' or form_data.get(mfa.name) == 'SETUP' or form_data.get('auth_method') == mfa.name:
                    current_app.logger.info(f"Setting up {mfa.name} authentication")
                    form_data[mfa.name] = 'SETUP'
                    # Clear any existing session data for this method
                    if mfa.name == 'email':
                        session.pop('mfa_email_code', None)
                        session.pop('mfa_email_id', None)
                        ...
                        session.pop('mfa_authenticator_opt', None)
                    view_result = mfa.registration_view(form_data)
                    current_app.logger.info(f"View result for {mfa.name}: {view_result}")
                    if view_result is not None:
                        mfa_view = view_result
                        break
                else:
                    current_app.logger.info(f"Setup not requested for {mfa.name}")
            else:
                current_app.logger.info(f"No matching form data for {mfa.name}")

    # Debug logging to see what's being passed to the template
    current_app.logger.info(f"Final MFA View: {mfa_view}")
    current_app.logger.info(f"Final MFA List: {mfa_list}")

    return Response(render_template(
        "mfa/register.html", _=_,
        mfa_list=mfa_list, mfa_view=mfa_view, next_url=next_url,
...
# Add this function to register the SMS routes
def register_sms_routes(blueprint):
    """Register SMS authentication routes."""
